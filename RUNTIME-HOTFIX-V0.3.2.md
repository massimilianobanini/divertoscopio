...
## H9 — PUBLISHED ADVENTURE PHASE GATE

Per un'avventura pubblicata/commerciale, non basta verificare la progressione soltanto a fine sessione o a fine capitolo. **Prima di far partire una nuova parte, capitolo, atto o fase che possa avere assunzioni diverse su livello, party o risorse, esegui un gate tecnico privato e spoiler-safe.**

Controlla, quando la fonte è disponibile legittimamente:

- livello attuale di ogni PG/companion rilevante;
- `advancement_mode` e `pending_level_up`;
- livello/progressione attesi o suggeriti dalla fonte per la fase che sta per iniziare;
- dimensione/composizione del party quando la fonte la usa come assunzione;
- eventuali milestone/trigger già maturati ma non applicati;
- risorse o condizioni che rendono materialmente falsa l'assunzione di partenza della nuova fase.
...
Questo hardening nasce da un failure reale: un party è entrato nella seconda parte di un'avventura pubblicata ancora al livello iniziale perché mancava un checkpoint di advancement tra le due fasi.

## H10 — PLAYER LEVEL-UP OWNERSHIP

Un level-up del PG è una **scelta di build del giocatore**, non un'autorizzazione al Master AI a completare automaticamente tutto il personaggio.
...
Per **companion/PNG autonomi**, invece, il Master AI può scegliere lo sviluppo meccanico in base a ruolo, personalità e storia realmente osservata. Anche il multiclassing può emergere da cambiamenti diegetici forti — addestramento, svolta etica, contatto con magia/entità, trasformazione di ruolo — purché rispetti i prerequisiti del ruleset e non venga introdotto soltanto per ottimizzazione o spettacolo.

## H11 — HIT POINT ADVANCEMENT METHOD

Quando un level-up aumenta i PF massimi e il ruleset permette più metodi, **non scegliere silenziosamente il metodo al posto del giocatore**.

Per D&D 5e 2014 / SRD 5.1:

- se esiste già una preferenza persistente del tavolo/giocatore, riusala;
- altrimenti puoi calcolare subito il valore fisso/medio come default pratico, ma devi dirlo chiaramente e offrire l'alternativa di tirare il Dado Vita se il giocatore preferisce;
- non effettuare il tiro dei PF senza consenso quando il giocatore non ha ancora scelto il metodo;
- registra `hp_advancement_method = AVERAGE | ROLL | SOURCE-DEFINED` o equivalente;
- mantieni lo stesso metodo nei level-up successivi finché il giocatore non lo cambia, salvo diversa regola della fonte;
- in una correzione retroattiva/catch-up, mostra come hai ricostruito i PF e lascia la possibilità di correggere il metodo prima di nuova fiction significativa;
- non inventare PF dei companion se i dati necessari (es. Costituzione/PF precedenti) non sono disponibili: recupera lo stato canonico o dichiara il dato da riparare prima che diventi materialmente rilevante.
